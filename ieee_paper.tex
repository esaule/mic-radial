...
\label{sec:expe}
In this section, we describe a series of numerical experiments to confirm 
the performance predictions of Section~\ref{sec:model}. To better
understand our results, we first perform some bandwidth measurements, followed
by the actual SPmV.  \todo{We might be criticized for not comparing with other work.}
...
grid). We also apply a Cuthill-Mckee reordering to all the matrices. 
The 2D and 3D grid sizes are chosen to have comparable matrix sizes. One notes
that the bandwidth reduction is higher in the 2D configuration, which leads
to better performance.  \todo{we do not show performance results in 2D. Do we
need to change anything? Do we have a reference for RCM?} 
This ordering technique is designed to reduce the distance between the 
nonzeros and the diagonal, hopefully allowing a better cache
...
\begin{figure*}
\centering
  \subfigure[Run on the MIC with 240 threads, 64-byte memory alignment.]{\includegraphics[width=.40\linewidth]{figures/bandwidth_read_write.pdf}\label{fig:band_rw}}
%
  \hspace{.3in}
%
  \subfigure[
    Run on the host with 32 threads, 32-byte memory alignment.]{\includegraphics[width=.40\linewidth]{figures/host_test1_readwrite_no_temporal_hint.pdf}\label{fig:read_write}}
...
\begin{figure}
\centering
  \subfigure[Performance of gather and unpack operations on the MIC. ] 
  {\includegraphics[width=.8\linewidth]{figures/bandwidth_gather_unpack.pdf}\label{fig:band_gather}} \\
\end{figure}
%
