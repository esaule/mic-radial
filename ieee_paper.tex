...
\todo{Add few sentence with discretization}
...
the reorder of the elements within each lane, while a permutation allows 
changes the order of entire lanes. 
...
is its own instruction. \todo{reference?}
...
the number of elements from the {\tt x} vector data transfered from
the memory is be minimum, they are transferred efficiently and remain 
in cache. The Compact matrices
...
grid). We also apply a Cuthill-Mckee reordering to all the matrices. This
ordering technique is designed to reduce the distance between the 
