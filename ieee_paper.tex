...
vectorial instruction. (We will show that an implementation relying on
compiler vectorization does not lead to desirable performance.) The
registers in the MIC architecture are 512 bits wide and can store 16
floats or integers. (In comparison to OpenCL or CUDA, one can think of
these registers as a warp.) The end goal is to be able to load and
format the non zero values $a,b,c,d$ and the vector entries $p,q,r,s$
in two vectorial register to be able to apply Fused Multiply-Add on
them, which is depicted in Figure~\ref{fig:tensor_product}. Bringing
the data in the vector register as depicted in the figure is the
difficult part that we now explain. A thread will perform the
multiplications one row at a time, and parallelism is achieved by
giving blocks of rows to each core usign an OpenMP construct.
...
support common reordering and broadcast.

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figures/swizzling.pdf}
  \includegraphics[width=\linewidth]{figures/channel_permutation.pdf}
...
vector. One can use lane permutation to broad a lane to take the whole
vector. So one 1 permutations and a quarter load are necessary to put
the data of the matrix in the required format.

We use the vector operations presented in
Figure~\ref{fig:MIC-vect-instr} in our code, which we'll explain in
the text.
...
Figure~\ref{code:read_aaaa} is a function that reads in four floats
(a,b,c,d) and creates the 16-float vector dddd,cccc,bbbb,aaaa:
...
__m512 read_aaaa(float* a)                                                              
{
    ...
    __m512 v1;
    v1 = _mm512_setzero_ps();
    v1 = _mm512_mask_loadunpacklo_ps(v1, mask, a);
    v1 = _mm512_mask_loadunpackhi_ps(v1, mask, a);
    v1 = _mm512_swizzle_ps(v1, _MM_SWIZ_REG_AAAA);
    return v1_old;
}
\end{verbatim}
\caption{read\_aaaa code}
\label{code:read_aaaa}
\end{figure*}

Figure~\ref{code:permute} is a function that takes four floats from
float register \ttt{v1}, and places them in each of the four
lanes. Because the permulation function only operates on 4-byte
integers, it is necessary to convert (in place) each float to an
integer (the bit structure is not changed), permute the lanes and cast
the integers back to floats (with no modification of the bit
structure)
