...
\begin{figure}[bth]
  ...
  \end{center}
  \caption{Notation relative to the matrices (upper section) and to the benchmarks (lower section.)}
  \label{tab:not}
\end{figure}
...
\begin{figure*}
  \centering 
  \subfigure[Absolute performance ]{\includegraphics[width=.49\linewidth]{figures/gflops_peak.pdf}\label{fig:gflops-peak-perf}}
%
...
\begin{figure*}[bth]
  \centering
  \subfigure[On MIC with 240 threads, 64-byte memory
    alignment.]{\includegraphics[width=.40\linewidth]{figures/bandwidth_read_write.pdf}\label{fig:band_rw_MIC}}
  %
  \subfigure[On CPU with 32 threads, 32-byte memory
    alignment.]{\includegraphics[width=.40\linewidth]{figures/host_test1_readwrite_no_temporal_hint.pdf}\label{fig:band_rw_CPU}}
...
\begin{figure*}
  \centering \subfigure[Performance of $y=Ax$ on the MIC. Squares:
    ...
    Cuthill-McKee bandwidth reduction. The grid is $96^3$]
...
%
  ...
  \caption{Vectorization matters in practice. Differences in BW
    translate in different performance.\todo{this two plots should ``correlate'' }\todo{legend}}
...
optimized implementation. On MIC (Figure~\ref{fig:perf_mic}) the
...
bound.  As a point of comparison, we show an AVX implementation on the
CPU of the two kernels and we show the best achievable performance in
the classical SpMV in Figure~\ref{fig:perf_host}. \todo{waiting for
  complete plot before writing more.}
...
%% Using a matrix with $96^3$ rows, we achieve a speedup of 7.7 relative
%% to the base case of one derivative and one function (1/1 case). We
%% have achieved a speed between 100 Gflop/s to 140 Gflop/s on 60 cores
%% using 240 threads, or a speed of 47\% to 66\% of the maximum possible
%% performance of this architecture. As a point of comparison, the
%% maximum possible speed for the 1/1 case is around 36 Gflop/s, of which
%% we achieve 18 Gflop/s. Our optimal implementation makes use of the
%% IMCI MIC instruction set which includes a number of swizzling and
%% channel swapping operations, for an extremely efficient tensor product
%% implementation. A straightforward implementation without vectorization
%% runs at around 35 Gflops, or 16\% of peak performance.
