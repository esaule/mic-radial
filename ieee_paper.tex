  ...
  \begin{center}
    \scalebox{.9}{
      \begin{tabular}{|c|l|}
        ...
        $s_M$ & size of the matrice(s)\\
        $s_x$ & size of the {\tt x} vector(s)\\
        $s_y$ & size of the {\tt y} vector(s)\\
        \hline
        $cl$    & size of a cache line \\
...
The matrix is composed of $n_r$ rows and columns with $n_z$ non-zero
per row leading to a total of $$n_c = n_r n_z$$ non-zero entries in
the matrix. Each of these non-zero entry have one index of size $b_i$
...
\begin{figure*}
  ...
  \centering 
  \subfigure[Peak performance of SpMV algorithm assuming either 1 or 4
    matrices and/or vectors. We used a memory bandwidth of 150
    Gbytes/sec (based on measurements in a code similar to the SpMV
    kernel, with computations
    removed).]{\includegraphics[width=.49\linewidth]{figures/gflops_peak.pdf}\label{fig:gflops_peak_perf}}
%
  \subfigure[Speedup relative to base case using one matrix and one
...
\begin{figure*}
  ...
  \subfigure[Bandwidth performance under idealized conditions as a
    ...
    instructions.]{\includegraphics[width=.49\linewidth]{figures/test1_readwrite.png}\label{fig:band_rw}}
%
  ...
  instructions.]{\includegraphics[width=.49\linewidth]{figures/test1_gather.png}\label{fig:band_gather}}
  ...
  \subfigure[Bandwidth performance under idealized conditions as a
    ...
    is aligned on 64
    bytes.]{\includegraphics[width=.49\linewidth]{figures/test3_gather.png}\label{fig:band_gather_ivdep}}
%
  \subfigure[Bandwidth performance on the host under idealized
    ...
    bytes.]{\includegraphics[width=.49\linewidth]{figures/host_test1_readwrite_no_temporal_hint.png}\label{fig:read_write}}
...
\begin{figure*}
  ...
  \subfigure[Performance of $y=Ax$ on the MIC. Squares: base 1/1 case,
    ...
    McKee.\todo{I don't think this is supercompact but RBF 3D.}]{\includegraphics[width=.49\linewidth]{figures/mic_performance_nb_threads.png}\label{}}
%
