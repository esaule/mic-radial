...
$\rvec$ and where the RBF is centered $\rvec_{j}$. That is, the
...
5\todo{Add few sentence with discretization. Check Erik}
...
$l$th matrix is at index $(k-1)*4+l-1$.  \todo{this description is innacurate.} The vectors {\tt x} are 
interleaved so that {\tt x}$^l_i$ and {\tt x}$^{l+1}_i$ are
consecutive in memory. The vector {\tt y} is similarly
interleaved, grouped in sets of 16, which correspond
