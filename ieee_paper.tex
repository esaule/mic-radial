...
goal is to be able to load and format the nonzero values $a,b,c,d$
...
allows to transfer 512 bits, so 16 floats at a time. A single nonzero
...
we need to load at once the data for four nonzeros in the matrix,
...
The values from the nonzero element in the matrix are the simplest to
...
complicated. First the columns pointers of 4 different nonzeros are
...
replicated so that each fill a vector lane using a swizzle
operation. Then each value is multiplied by 4 (because there are 4
vectors) and the offsets $(3,2,1,0)$ is added each lane to obtain the
...
performed bring the 16 entries of the {\tt x} vector into the SIMD
...
permutation, one can replicate an entry of the four vectors accross
...
Order hint to optimize traffic on the memory bus.
...
\centering
  \subfigure[
    ...
    instructions.]{\includegraphics[width=.40\linewidth]{figures/bandwidth_read_write.pdf}\label{fig:band_rw}}
%
  \hspace{.3in}
%
  \subfigure[
