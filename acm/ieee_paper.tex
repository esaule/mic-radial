...
set that show an almost \ge{sevenfold} \note{eightfold; there were inconsistencies in the ppaer.} increase in speed compared to
...
A performance of 135~Gflop/s in single precision is achieved using RBF-FD differentiation matrices, which is 3.75 imes better than the theoretical peak performance of an SpMV operation. Concluding remarks and perspectives are provided in Section~\ref{sec:ccl}.\looseness=-1
...
\subfigure[Content of the vector registers \ge{in the source code in (a)}.]{\hspace{-0.07\linewidth}\includegraphics[width=.5\linewidth]{figures/code-annote.pdf}}
...
To put things in perspective, we conducted similar experiments on a
